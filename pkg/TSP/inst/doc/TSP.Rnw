...
in a distance matrix $\mathbf{D}$ with elements $d_{ij}$ where $i,j = 1\dots n$
...
<<eval=FALSE, label=clustering_code>>=
...
<<echo=FALSE, fig=TRUE, include=FALSE, label=clustering>>=
e <- bitmap(file="TSP-clustering.png", type="png16m", res=600,
height = 6, width = 6, pointsize=11)
<<clustering_code>>
e <- dev.off()
@
