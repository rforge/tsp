...
In the following we show the 20 cities before and 20 after the dummy city.

<<>>=
#labels(tour)
dummy <- which(labels(tour)== "dummy")
labels(tour)[(dummy-20):(dummy+20)]
@

The tour found in the example results in a path from Lihue in Hawaii to
Yellowknife in Canada's Northwest Territories. It has to be mentioned, that the
