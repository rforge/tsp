...
which is the \emph{length} of the tour.
...
As an example we show the first tour. Then 
the tour lengths are compared using the dot chart in Figure~\ref{fig:dotchart}.
The vertical line in the plot represents the optimal solution with a tour
...
dotchart(sapply(tours, FUN = attr, "tour_length"), 
xlab = "tour length", xlim = c(0, 25000))
## add optimum
opt <- 14497
...
The tour found in the example results in a path from Lihue in Hawaii to
Yellowknife in Canada's Northwest Territories. Such a tour can also be 
visualized using the packages \pkg{sp}, \pkg{maps} and \pkg{maptools}.

<<fig=TRUE, include=FALSE, label=map1>>=
library("maps")
library("sp")
library("maptools")
...
plot_path <- function(path){
    ...
    line <- SpatialLines(list(Lines(Line(USCA312_coords[path,]))))
...
We use the nearest neighbor heuristic 
...
<<fig=TRUE, include=FALSE, label=map2>>=
...
ends in Hawaii. The path shows the typical behavior 
...
<<fig=TRUE, include=FALSE, label=map3>>=
tour <- solve_TSP(atsp, method ="nearest_insertion")
tour
...
We insert two dummy cities into the TSP for the iris data set (the class
attribute called \code{"Species"} is used as city labels) and perform
...
data given by the species field in the data set. Since we used the species as 
the city labels in the TSP, the labels in the tour 
represent the partitioning with the dummy cities named `boundary' seperating
groups.
...
The resulting clustering perfectly splits the iris data set into a group
...
this method fails to find the three groups in the data set. The reason is the
