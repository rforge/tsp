...
The problem of finding the shortest Hamiltonian path through a graph can
be transformed into the TSP~\citep{Garfinkel1985} with cities and
distances representing the graphs vertices and edge weights,
respectively.
...
into the TSP for the iris data set and perform rearrangement clustering
...
the label for the cities in the TSP, the labels in the tour 
represent the partitioning with the dummy cities names boundary seperating
groups.
