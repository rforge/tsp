...
same. In the asymmetric case the distances are not equal for all cities pairs. 
...
\begin{itemize}
    ...
    \item \func{image} produces a shaded matrix plot of the dissimilarities
...
A popular format to save TSP descriptions to disk which is supported by most TSP
...
The currently available methods are described in
Section~\ref{sec:algo}.

\section{Algorithms}\label{sec:algo}

The NP-hardness of the TSP make it already for medium sized necessary to rely
...
greedy procedure: The algorithm starts with a tour containing a random city.
Then the algorithm always adds to last city in the tour the nearest not yet
visited city. The algorithm stops when all cities are on the tour.
It is implemented as method \code{"nn"}.

An extension to this algorithm is to repeat it with each city as
the starting point and then choose the best tour. 
This algorithm is implemented as \code{"repeated\_nn"}.

\subsection{Insertion algorithms}
All insertion algorithms~\citep{Rosenkrantz1977} start with a tour consisting
of an arbitrary city and choose in each step a city $k$ not yet on the tour.
This city is inserted into the existing tour between two consecutive cities
$i$ and $j$, such that 
$$d(i,k) + d(k,j) - d(i,j)$$ 
is minimized.  The algorithms stops when all cities are on the tour.

The insertion algorithms differ in the way the the city to be inserted next is
chosen. 
The following insertion algorithms are:
...
the city which is \emph{farthest} to a city on the tour.
...
spanning tree algorithm where adding a city to a partial tour corresponds to
adding an edge to a partial spanning tree.  Here the TSP is solved by finding
...
using depth first traversal of the tree~\citep{Rosenkrantz1977}.  For distances
which obey the triangular inequality the upper bound for the length of the
tour found by the minimum spanning tree algorithm is twice the optimal tour
length which also holds for the nearest and cheapest insertion algorithms.

The idea behind the farthest insertion algorithm is to link cities far away
...
However, it can be shown that the algorithm generates tours which approach
$1.5$ times the optimal tour length~\citep{Rosenkrantz1977}.
...
\pkg{TSP} provides a simple interface to Concorde
which is used for method \code{"concorde"}.
. It saves the TSP using
...
Finding the shortest Hamiltonian which ends in a given city can be achieved
...
boundaries of $k$ clusters by adding $k$ \emph{dummy cities} which has
