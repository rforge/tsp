...
transform the data set into an asymmetric TSP and set the column corresponding
to New York to zero before solving the TSP. This means that the distance to
...
ends in a city in Manitoba, Canada. 
...
tour <- solve_TSP(atsp, method ="nearest")
tour
