...
which are important fo the rest of the paper.
...
A different manipulation is to reformulate a asymmetric TSP $D$
as a symmetric TSP $D'$.
...
\subsection{Solving the TSP}\label{sec:algo}

Exact solutions for a TSP can be calculated using dynamic
programming and branch-and-bound procedures.  

For solving a TSP by dynamic programming~\citep{Held1962}\dots
