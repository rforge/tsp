...
the TSP can be regarded as a complete graph $G = (V, E)$, where the
cities correspond to the node set  $V = \{1,2,\cdots,n\}$
and each edge $e \in E$ has an associated weight representing the 
distance between the nodes it connects.
If the graph is not complete, the missing edges can be
replaced by edges with very large distances.
The goal is then to find a \emph{Hamiltonian cycle} with the least
weight in the graph~\citep{Hoffman1985}.
...
\section{The TSP infrastructure}\label{sec:infrastructure}
...
\section{Algorithms and heuristics}\label{sec:algo}
...
for larger instances branch-and-cut.
...
In the following, we present the differnt algorithms.
