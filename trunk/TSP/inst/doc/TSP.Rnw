...
Other representations are as integer and linear programming 
problems~\citep[see, e.g.,][]{Punnen2002}. 
The \emph{integer programming (IP) formulation} 
...
The solution $\mathbf{x} = (x_{ij})$ of the assignment problem represents a
tour or a collection of subtour (several unconnected cycles) where all edges
which correspond to the elements $x_{ij}$ which are $1$ are in the tour or a
subtour. Additional restrictions, called \emph{subtour elimination
...
The \emph{linear programming (LP) formulation} 
is given by:
\[
\begin{array}{rl}
...
\end{array}
\]
where $m$ is the number of edges in $G$, $w_i$ is the weight of edge $e_i$ and
$\mathbf{x}$ is the incidence vector indicating the presence or absence of each
edge in the tour. Again, the constraints given by $\mathbf{x} \in \mathcal{S}$
are problematic since they have to contain the set of incidence vectors of all
possible Hamiltoninan cycles in $G$ which amounts to a direct search of all
possibilities and thus is infeasible. However, relaxed versions of the linear
programming problem with removed integrality and subtour elimination
constraints are extensively used by modern TSP solvers where this partial
descriptions of contraints is used and improved iteratively in a
...
\subsection{Exact solutions to the TSP}\label{sec:exact}
...
instances. To find the optimal tourm, a permutation $\pi = \{1, i_2,
...
However, for very small TSP instances the approach is fast.
...
ineqaulity systems is called \emph{cutting-plane method} and was introduced by
\cite{Dantzig1954}. 

Each iteration begins with using using instead of the original linear
inequality description $\mathcal{S}$, $A\mathbf{x} \le b$ where the polyhedron
$P$ defined by the relaxation contains $\mathcal{S}$ and is bounded.
The optimal solution $\mathbf{x}^*$ of the relaxed problem can be
found using standard linear programming solvers. If $\mathbf{x}^*$ belongs to
...
cutting-plane. A family of cutting-planes can be added to the inequality
system  $A\mathbf{x} \le b$ to get a tighter relaxation which can be used in
the next iteration. 
...
without further branching or is found to be irrelveant because its relaxed
version already produces a longer path than another already found solution.
This method is calles \emph{branch-and-cut} which is a variation of the well
known \emph{branch-and-bound} procedure.
% cite Little and Murty...
% cite
...
$\mathbf{x}$ for which all elements $x_e$  satisfy $0 \le x_e \le 1$ and in the
resulting tour each city is linked to exactly two other cities.  Variouse
separation algorithms for finding subsequent cuts to prevent subtours
(\emph{subtour elimination inequalities}) and to ensure that $\mathbf{x}^*$ is
an integer vector (\emph{Gomory cuts}~\citep{Gomory1963}) where developed over
time. The currently most efficient implementation is described 
...
comprehensive overview of TSP heuristics including an experimental comparison,
we refer the reader to the book chapter by \cite{Johnson2002}.

The tour construction heuristics are the nearest neighbor algorithm and
the inseartion algorithms.
...
Tour improvement heuristics are based on exchanging edges in a initial tour.
...
\begin{table}
    \caption{Available algorithms.}\label{tab:methods}
    \centering
    \begin{tabular}{ll}
    \hline
...
discussed in section~\ref{sec:exact}.
