...
these solvers, we can formulate the ATSP as a TSP using \func{ATSP\_to\_TSP}
...
groups is the closeness between the groups `Virginica' and `Versicolor'. This
can be clearly seen in Figure~\ref{fig:clustering2}, where the data points are
projected on the first two principal components of the data set.  In the
figure, we added the path segments which resulted from solving the TSP.  All
points connected by a single path represent a cluster.  Clearly, the two groups
to the right side of the plot are too close to be separated correctly.
...
\begin{figure}
\centering
\includegraphics[width=12cm]{TSP-clustering2}
