...
For the minimization of the tour length, $(n-1)!$ permutation vectors
have to be compared which results in a very hard to solve problem known to be
NP-complete~\citep{Johnson1985a}.
...
\marginpar{Should we use matrix notation?} 
\[
\begin{array}{rl}
...
\marginpar{Explain manipulations for paths here?}
...
can be found in the chapter by \cite{Rego2002}.
...
solution. Further improvements to the procedure are described by \cite{Lin1973}.
...
\func{ATSP\_to\_TSP} is provides. The function creates the necessary dummy
cities and adapts the distance matrix accordingly. 
...
License (GPL). For the Concorde TSP solver and the Chained Lin-Kernighan
...
ends in the northeast of Canada. 
