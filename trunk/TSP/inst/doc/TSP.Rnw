...
have to be compared. This results in a very hard to solve problem which is
known to be NP-complete~\citep{Johnson1985a}.
...
measures which obey the triangle inequality are made.  An important distincion
...
in $D$ the equality $d_{ij} = d_{ji}$ holds, i.e., it does not matter if we
...
formulation} is based on the assignment problem which is
...
that the integer programming problem in itself is NP-hard, also the number of
...
into a diffeeent matrix $D' = (d'_{ij})$ which has the same optimal solution.
...
recursively and the minimal tour length $d^{**}$ can be found.  In a second
step, the optimal permutation $\pi = \{1, i_2, i_3,\dots,i_n\}$ of city indices
$1$ through $n$ can be computed in reverse order, staring with $i_n$ and
working successively back to $i_2$. The procedure exploits the fact that a
permutation $\pi$ can only be optimal, if
...
optimal solution suffices.  TSP heuristics typically fall into two groups, tour
...
heuristic (a Lin-Kernighan variation described in \cite{Applegate2003}) are
included in the package and distributed under the GNU Public License (GPL). For
the Concorde TSP solver and the Chained Lin-Kernighan heuristic only an simple
interface (using \func{write\_TSPLIB}, calling the executable and reads back
the resulting tour) is included in \pkg{TSP}.  The code itself is part of the
Concorde distribution, has to be installed separately and is governed by a
different license which allows only for academic use.  The interfaces where
includes since Concorde~\citep{Applegate2000,Applegate2006} is currently one of
the best implementations for solving symmetric TSPs based on the branch-and-cut
method discussed in section~\ref{sec:exact}.  In May 2004, Concorde was used to
find the optimal solution for the TSP of visiting all 24,978 cities in Sweden.
The computation was carried out on a cluster with 96 nodes and took in total
almost 100 CPU years (assuming a single CPU Xeon 2.8 GHz processor).
...
%Since Concorde and many advanced TSP solvers can only solve symmetric TSPs, we
%have to formulate the ATSP as a TSP. This can be done using
%\func{ATSP\_to\_TSP} which doubles the number of cities (see
%Section~\ref{sec:manipulations}).  After finding a tour for the TSP, the dummy
%cities with have to be removed again.  The following code is not executed here,
%since it takes several minutes and concorde has to be installed separately.
%Concorde finds the optimal solution with a length of 39845.
%
%<<>>=
...
%@
%<<eval=FALSE>>=
%#tour2 <- solve_TSP(tsp2, method = "linkern")
%tour <- solve_TSP(tsp, method = "concorde")
%tour <- as.TOUR(tour[tour <= n_of_cities(atsp)])
%tour_length(atsp, tour2)
%@
